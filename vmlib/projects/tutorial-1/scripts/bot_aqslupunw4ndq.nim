...
- setup:
  ...
  speed = 10
  turn player
  sign.position = position + (UP * 2.0) + (RIGHT * 1.2)
...
- movement_info:
  say "...", 0.5, 0.5, """
    ...
    Enu is a 3D world where you can code and create almost anything using a programming language called [Nim](https://nim-lang.org). It's a place to make games, experiment, and learn to code. You're using an early prototype that's missing important features and is full of bugs, but it's getting better quickly. Please contact me at [dsrw@me.com](mailto:dsrw@me.com) if you have any questions or suggestions.

    In this tutorial you'll learn to move and do some simple coding. In later tutorials we'll make towers, tunnels, characters, and will finish with a 3D survival game called `Inky: Isolation`.
    ...
    You can close this window by pressing `ESC` on your keyboard or `B` on your controller, and can come back at any time by clicking on the text block next to the `Bot`. You can also move without closing the window by using your controller, or by holding down `alt/option (⌥)` on your keyboard.
...
- tool_info:
  say "- Switch to `Place Bot`", 3.0, 1.0, """
    ...
    Enu currently has 8 tools. Tool `1` on the left is the `Code` tool. It can program almost anything in Enu, and will be explained in detail later. Tool `8` on the right is the `Place Bot` tool. Use it to fill your world with friendly robots. Tools `2` - `7` are the `blue`, `red`, `green`, `black`, `white`, and `brown` colored blocks.
...
- code_info:
  say("- Coding", 2.0, 0.5, """
    ...
    We control Enu using [Nim](https://nim-lang.org), a powerful programming language that's useful for a wide variety of tasks. Almost everything in Enu can be coded with Nim. Let's start by teaching our `Bot` to walk to the end of the course.
...
- bot_navigation:
  say(body = """
    ...
    We control the bot with simple commands like `forward` and `turn`. Here's some code to get our `Bot` partway through the course:
    ...
    Run your code by pressing `alt/option (⌥)`. Keep `alt/option (⌥)` held down to move around while you're coding. Flying is a great way to get yourself into position so you can better see what's happening, so try double jumping to get a better view.
    ...
    Good luck! We'll conclude this tutorial when your `Bot` goes past the `FINISH HERE!` sign at the end of the course.
...
- win:
  ...
  say "- All Done!", 2.5, 0.5, """
    ...
    Good work! You learned the basics of Enu, and you coded your first robot. There's a lot more to come. Everything here - the signs, the course, the giant spinning confetti monster, were created with `Nim` code inside Enu, and we'll cover how to do all of it in future tutorials.

    This is supposed to be where you move to the next tutorial, but it hasn't been written yet. For now, click [HERE](<nim://player.god = true;sign.open = false>) to turn on `God Mode`, which will let you see and modify the code for this tutorial. It's a bit of a mess, but some folks might find it interesting. Or click [HERE](<nim://press_action("next_world")>) to load `Inky: Isolation`, a simple game written in Enu that's loosley inspired by **Alien: Isolation**, but features Inky, the blue ghost from Pac-Man.

    Contact me at [dsrw@me.com](mailto:dsrw@me.com) if you have any questions or suggestions. Thanks for trying Enu!
